...
-- =============================================
CREATE PROCEDURE SP_GenerarHistoricoDiario 
...
insert into @FirstTable 
	...
	DECLARE @SecondTable TABLE (IdConfigIndicadorResponsable INT,Prefijo nvarchar(300), 
			...
			Semaforo nvarchar(100), IdResponsable int, EstatusSubEtapa bit,FechaInicioEtapa date, FechaInicioSubEtapa date,DayGenerated date,
			NombreResponsable nvarchar(MAX)
			)
		...
		insert into tbl_Historico 
			...
			f.IdResponsable,
			EstatusSubEtapa,
			(Select c.FechaInicio from Config_FechasEtapasProyecto c where IdProyecto=f.IdProyecto and IdEtapa=f.IdEtapa) as FechaInicioEtapa,
			(Select s.FechaInicio from Config_FechasSubEtapasProyecto s where IdProyecto=f.IdProyecto and IdSubEtapa=f.IdSubEtapa) as FechaInicioSubEtapa,
			GETDATE(),
			(select NombreResponsable from tbl_Responsables where idResponsable=f.IdResponsable)
